...
<br>
                                        ...
                                        <div class="form-group">
                                            <label class="control-label col-md-3 col-sm-3 col-xs-12">B-Partner</label>
                                            <div class="col-md-9 col-sm-9 col-xs-12">
...
{ 
     ...
     $.ajax({type: "GET",url: "http://vpn.spiderg.com:8000/messagingurl",contentType:'application/json',headers: {'SPIDERG-API-Key': 'e5e3b300-31e9-4ad2-a705-4f8935218fcb','SPIDERG-Authorization': "SPIDERGAUTH "+ createAuthenticationHeader(username,password,loginToken,loginTokenTS)},success: function (data){
		...
		if(msg.refid=="adv"){ 
				...
				var d = new Date(msg.history[k].timestamp); 
		...
		var channel="";
          $(document).ready(function(){
            ...
            spiderG.getLoginToken(username, function()
            {
                ...
                $.ajax({
                    type: "GET",
                    url: "http://vpn.spiderg.com:8081/SpiderGAPIServer/api/rfq/"+org_id,
                    contentType:'application/json',
                    headers: { 
                    ...
                    success: function (data)
                    {
                       ...
                       channel=people.channel;
                        $("#doc_no").html(people.documentno);
						  $.ajax({type:"GET",url:"http://vpn.spiderg.com:8081/SpiderGAPIServer/api/mailbox?dtype=rfq",contentType:'application/json',headers: {'SPIDERG-API-Key': 'e5e3b300-31e9-4ad2-a705-4f8935218fcb','SPIDERG-Authorization': "SPIDERGAUTH "+ createAuthenticationHeader(username,password,loginToken,loginTokenTS)},success:function (data){
						mailboxlist = JSON.parse(data); 
						for(var j=0; j< mailboxlist.length; j++)
						{ if(mailboxlist[j].documentid==people.id)
							{ 
							console.log(mailboxlist[j]);
							mailid=mailboxlist[j].id;
						      if(mailboxlist[j].accepted=="Y"){
							  $('#acceptpo').val("Reject Enquiry");
							  }else
							  {
							  $('#acceptpo').val("Accept Enquiry");
            ...
            function getorgname(org)
            {
                ...
                $.ajax({
                    type: "GET",
                    url: "http://vpn.spiderg.com:8081/SpiderGAPIServer/api/org?orgid="+org,
                    contentType:'application/json',
                    headers: { 
                            'SPIDERG-API-Key': 'e5e3b300-31e9-4ad2-a705-4f8935218fcb',
                            'SPIDERG-Authorization': "SPIDERGAUTH "+ createAuthenticationHeader(username,password,loginToken,loginTokenTS)
