...
<body class="nav-md">
  ...
  <input type="hidden" id="uom_name">
        ...
        {   
            ...
            spiderG.getLoginToken(username, function()
            {
              ...
              $.ajax({
                  type: "GET",
                  url: "http://vpn.spiderg.com:8081/SpiderGAPIServer/api/product/"+pro_id,
                ...
                success: function (data ,textStatus) 
                {
                   ...
                   $('#p_nameold').val(people.name);
                    ...
                    //$('#uom_id').val(people.uom);
                    $('#pro_cat').val(people.category_id);
                    // alert('Success');
    				...
    				if(people.ispublic == 'Y' )
                    {
						$('#sold_y').attr('checked',true); 
				    }
    				else
    				{
    					$('#sold_n').attr('checked',true); 
    				}

    				if(people.issold == 'Y' )
                    {
						$('#public_y').attr('checked',true); 
				    }
    				else
    				{
    					$('#public_n').attr('checked',true); 
    				}
                    get_uomname(people.uom);
                    //get_categoryname(people.category_id);
                ...
                $.ajax({
                    ...
                    success: function (data)
                    {
                        ...
                        for(var j=0; j< people.length; j++)
                        {
                            ...
                            //$('#drpdwn').append('<option value="'+ people[j].id +'" if(people[j].id == curnt_id) selected>'+ people[j].name +'</option>');
                       }
                    },
                error: function (err) {
                console.log(err);
 ...
 //});
        ...
        $( document ).ready(function()
        {
            ...
            success: function (data){
                people = data; 
                var curntpro_id = $('#pro_cat').val();
                for(var j=0; j< people.length; j++)
                {
                  // $("#catdrpdwn option:selected").val(curntpro_id);
                   // $('#catdrpdwn').append('<option value="'+ people[j].id +'" if(people[j].id == curntpro_id) selected > '+ people[j].name +'</option>');
