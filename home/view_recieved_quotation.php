                                ...
                                <div class="x_content">
                                   <!--  <div class="col-md-6"> -->
										...
										<div class="form-group col-md-12">
                                            ...
                                            <label style="margin-left: -1%;" class="control-label col-md-3 col-sm-3 col-xs-12">Freight charges</label>
                                            <div class="col-md-9 col-sm-9 col-xs-12">
                                                <span id="freightcharge"></span>
                                            </div>
                                        </div>
											<div class="form-group col-md-12">
                                        ...
                                        <div class="form-group col-md-12">
                                            <div class="col-md-9 col-sm-9 col-xs-12">
                                                <span id="cstmlink"></span>
												  <input type="button" class="btn btn-primary"  id="acceptpo" value="Accept Quotation"> 
                                            </div>
                                        </div> 
                                </div> </div>
...
{ 
	 ...
	 if($('#acceptpo').val()=="Accept Quotation"){
        ...
        <script>var channel="";
          $(document).ready(function(){
            ...
            spiderG.getLoginToken(username, function()
            {
                var loginToken = spiderG['loginToken'];
                var loginTokenTS = spiderG['loginTokenTS'];
                $.ajax({type: "GET",url:"http://vpn.spiderg.com:8081/SpiderGAPIServer/api/quotation/"+org_id,contentType:'application/json',headers:{'SPIDERG-API-Key':'e5e3b300-31e9-4ad2-a705-4f8935218fcb','SPIDERG-Authorization':"SPIDERGAUTH "+createAuthenticationHeader(username,password,loginToken,loginTokenTS)},success: function (data){
                        people = data; 
						...
						mailboxlist = data; 
									...
									if(mailboxlist.accepted=="Y"){$('#acceptpo').val("Reject Quotation");}else
									{ $('#acceptpo').val("Accept Quotation");} }}); 
						...
						$("#freightcharge").html(people.freightamt);
    ...
    if(people.lineitems[0]){ $("#netamount").html(people.grandtotal-people.lineitems[0].taxamt); }
    if(people.lineitems[0]){ $("#totalamount").html(people.grandtotal); }
    if(people.lineitems[0]){ $("#taxamount").html(people.lineitems[0].taxamt); }
    ...
    $("#notes").html(people.notes); 
        getdeliveryloc(people.deliverylocation);
                        ...
                        $("#cstmlink").html("<a href='purchase_order.php?id="+org_id+"' class='btn btn-primary'>Send PO</a>");
                    },
                    error: function (err) {
                    console.log(err);
