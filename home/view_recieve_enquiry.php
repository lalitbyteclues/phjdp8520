...
<br>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-9 col-sm-9 col-xs-12">
												...
												<input type="hidden" value="" id="mailboxid">
                                            </div>
                                        </div>
                                            <br>
                                            <br>
                                        ...
                                        <div class="form-group col-md-12">
                                            ...
                                            <span id="cstmlink"></span>
                                               <input type="button" class="btn btn-primary"  id="acceptpo" value="Accept Enquiry"> 
                                            </div>
                                        </div>
                                    <!-- </div> -->
                                </div> </div>
...
{ 
     ...
     $.ajax({type: "GET",url: "http://vpn.spiderg.com:8000/messagingurl",contentType:'application/json',headers: {'SPIDERG-API-Key': 'e5e3b300-31e9-4ad2-a705-4f8935218fcb','SPIDERG-Authorization': "SPIDERGAUTH "+ createAuthenticationHeader(username,password,loginToken,loginTokenTS)},success: function (data){
		 ...
		 socket.on('connect', function () { console.log('Client has connected to the server!');console.log(channel);}); 
          ...
          $(document).ready(function(){
            ...
            spiderG.getLoginToken(username, function()
            {
                ...
                $.ajax({type:"GET",url:"http://vpn.spiderg.com:8081/SpiderGAPIServer/api/rfq/"+org_id,contentType:'application/json',headers:{'SPIDERG-API-Key':'e5e3b300-31e9-4ad2-a705-4f8935218fcb','SPIDERG-Authorization': "SPIDERGAUTH "+ createAuthenticationHeader(username,password,loginToken,loginTokenTS)},success: function (data){
				  ...
				  $.ajax({type:"GET",url:"http://vpn.spiderg.com:8081/SpiderGAPIServer/api/mailbox/rfq/"+people.id,contentType:'application/json',headers: {'SPIDERG-API-Key': 'e5e3b300-31e9-4ad2-a705-4f8935218fcb','SPIDERG-Authorization': "SPIDERGAUTH "+ createAuthenticationHeader(username,password,loginToken,loginTokenTS)},success:function (data){
						mailboxlist = data;  
							mailid=mailboxlist.id; 
							$("#mailboxid").val(mailboxlist.id);
							mailid=mailboxlist.id; 
							getorgname(mailboxlist.senderorg);
						      if(mailboxlist.accepted=="Y"){
							  $('#acceptpo').val("Reject Enquiry");
							  }else
							  {
							  $('#acceptpo').val("Accept Enquiry");
							  } 
				  }}
				  );
                 ...
                 if(people.lineitems[0]){ getuom(people.lineitems[0].uom); }
                     $("#cstmlink").html("<a href='quotation.php?id="+org_id+"' class='btn btn-primary'>Send Quotation</a>");
                    },
                    error: function (err) {
                    console.log(err); 
                    }
 ...
 spiderG.getLoginToken(username, function()
            {
				  var loginToken = spiderG['loginToken'];
                var loginTokenTS = spiderG['loginTokenTS'];
	 if($('#acceptpo').val()=="Accept Enquiry"){
		 console.log(mailid);
		 
		 if(mailid==""){mailid= $("#mailboxid").val(); } 
	 $.ajax({type: "POST",url: "http://vpn.spiderg.com:8081/SpiderGAPIServer/api/mailbox/accepted/"+mailid,contentType:'application/json'
			 ...
			 $.ajax({type: "POST",url: "http://vpn.spiderg.com:8081/SpiderGAPIServer/api/mailbox/rejected/"+mailid,contentType:'application/json',headers:{'SPIDERG-API-Key': 'e5e3b300-31e9-4ad2-a705-4f8935218fcb','SPIDERG-Authorization': "SPIDERGAUTH "+ createAuthenticationHeader(username,password,loginToken,loginTokenTS)},success: function (data){ 
						$('#acceptpo').val("Accept Enquiry");
			 }});
			 } });
			 });
